...
| `HERMES_HOST_APP_NAME` | Name of the desktop app that hosts Hermes (for example `Memoe`), used in fixable-failure messages that tell the person which app to turn on in System Settings ("Turn Memoe on in … Full Disk Access"). Unset, those messages say "the app". Set by the host, not by users; see `tools/fix_reasons.py` for the error contract |
...
| `HERMES_TOOL_BRIDGE_SOCKET` / `HERMES_TOOL_BRIDGE_TOKEN` / `HERMES_TOOL_BRIDGE_TOOLS` | Set by `ClaudeCodeSession` in the `env` block of the `--mcp-config` it writes, addressing the [tool bridge](../integrations/providers#subagents-and-memory) that carries `delegate_task` / `memory` / `session_search` / `todo` back to the Hermes process. The socket and token are minted per session; `_TOOLS` is the comma-separated subset that session's agent may actually run. Don't set manually. |
| `HERMES_TOOL_BRIDGE_TIMEOUT` | Seconds the tool bridge's client waits for the parent to answer (default: `1500`). Must stay strictly below the CLI's `MCP_TOOL_TIMEOUT` so a call that runs too long comes back as a readable tool error rather than an opaque MCP abort. |
| `MCP_TIMEOUT` / `MCP_TOOL_TIMEOUT` | Read by the `claude` CLI, not by Hermes. `build_child_env` sets them (180 000 ms / 1 800 000 ms) because the `hermes-tools` server imports the whole tool registry at startup and a bridged `delegate_task` runs subagents to completion before answering; the CLI's own defaults would kill both. `setdefault`, so an operator's values win. |
