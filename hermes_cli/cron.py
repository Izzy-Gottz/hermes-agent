...
def _print_ticker_health(pids: list, via_heartbeat: bool = False) -> None:
    ...
    ``via_heartbeat``: the heartbeat was the SOLE witness (no pid, no lock) — say so rather
    than claiming a process nobody observed.
    """
    # See #32612, #32895.
    ...
    # Shared with the liveness probe so the two cannot drift (200s @ 60s default).
    STALE_AFTER = _ticker_stale_after_seconds()
    ...
    if hb_age is None:
        ...
        if via_heartbeat:
            print(color("✓ Cron ticker is running — jobs will fire automatically", Colors.GREEN))
            print(color(
