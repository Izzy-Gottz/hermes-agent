            ...
            try:
                ...
                for tool_name in preledger_tools:
                    try:
                        ...
                        # deregister() picks its partition from the CALLING
                        # module's plugin ownership, and ``hermes_cli.plugins``
                        # is not itself a plugin — so the caller-derived scope
                        # is None and the lookup lands in the process-global
                        # map. Plugin tools are never there: scope_key is never
                        # None, even single-profile. The zombie-entry sweep this
                        # loop exists to be (#60050) had therefore always been a
                        # silent no-op.
                        tool_registry.deregister(tool_name, scope=self.scope_key)
