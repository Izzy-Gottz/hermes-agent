...
def _preflight_check_skills(job: dict) -> Optional[str]:
    """Block only on an affirmative verdict from ``skill_view`` — the skill exists and either its required
    environment is missing (``setup_needed``) or a command it declares is not on this machine. Both are runs
    that are guaranteed to misfire. Skills that fail to load fall through to ``_build_job_prompt``'s
    skipped-skill handling (fail-open).

    The command half is the stricter one and lives HERE rather than in ``skill_view``'s ``setup_needed``:
    interactively a person can install the binary and try again, so relabelling the skill everywhere would be
    too blunt (``agentmail`` declares a command and deliberately keeps its key optional so self-signup stays
    open). A scheduled run has nobody to ask — it spends an LLM call and dies at the shell — so that is where
    an absent binary is worth refusing over. It used to be consulted only when some OTHER requirement had
    already set ``setup_needed``, so it could never fire on its own.
    """
