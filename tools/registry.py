...
def check_fn_cache_scope() -> Optional[str]:
    ...
    try:
        ...
        if all(str(get_session_env(k, "") or "").strip() for k in _BROWSER_IDENTITY_KEYS):
            # That bypass is gated on the browser-control feature flag. The
            # api_server binds a session id, a server-derived principal and a
            # transport family for EVERY request (``_bind_api_server_session``),
            # whether or not any browser controller exists — so an unconditional
            # bypass made every API turn rebuild the full tool surface twice
            # (agent init + the between-turns MCP refresh), ~120 ms each with
            # ~2000 lazy MCP tools registered, and left both caches permanently
            # empty. With ``browser.extension_control.enabled`` off,
            # ``extension_controller_available`` returns False before it looks
            # at the bound identity, so nothing request-bound can change
            # between turns and the ordinary process-wide cache is exactly as
            # correct as it is for the CLI. Fail closed (bypass) if the flag
            # cannot be read.
            try:
...
class ToolRegistry:
    ...
    def _drop_toolset_aliases(self, toolset: str) -> None:
        """Remove GLOBAL aliases pointing at *toolset* (see ``_drop_aliases_for_toolset``)."""
        self._drop_aliases_for_toolset(toolset, None)

    def _drop_aliases_for_toolset(self, toolset: str, scope: Optional[str]) -> None:
        ...
        if not part:
            return
        remaining = {alias: target for alias, target in part.items() if target != toolset}
        if remaining:
    ...
    def register_toolset_alias(self, alias: str, toolset: str, *, scope: Optional[str] = None) -> None:
    ...
    def get_registered_toolset_aliases(self, *, scope: Optional[str] = None) -> Dict[str, str]:
        """Return a snapshot of ``{alias: canonical_toolset}`` mappings."""
        with self._lock:
            return self._merged_aliases(scope)

    def get_toolset_alias_target(self, alias: str, *, scope: Optional[str] = None) -> Optional[str]:
    ...
    def deregister(self, name: str, *, scope: Optional[str] = None) -> None:
        ...
        with self._lock:
            ...
            if not self._toolset_entries(entry.toolset, scope):
                ...
                self._drop_aliases_for_toolset(entry.toolset, scope)
            self._generation += 1
        ...
        with self._lock:
            ...
            for toolset in affected_toolsets:
                ...
                if not surviving and not any(e.toolset == toolset for e in in_overlays):
                    # NO entry survives in any partition, so the alias is
                    # orphaned everywhere. Dropping only ``scope`` left a
                    # plugin's alias behind forever: plugins register aliases
                    # globally while their tools are always scoped
                    # (PluginManager's scope_key is never None), so the
                    # partition-local drop found nothing and
                    # ``validate_toolset`` kept answering True for a toolset
                    # with zero tools.
                    self._drop_aliases_for_toolset(toolset, None)
    ...
    def dispatch(
        ...
        try:
            ...
            from tools.fix_reasons import as_tool_error, fields_of
            if fix := fields_of(e):  # only a code IN the vocabulary skips the generic wrapper
                ...
                logger.info("Tool %s: fixable failure %s", name, fix["code"])
    ...
    def get_toolset_for_tool(self, name: str, *, scope: Optional[str] = None) -> Optional[str]:
