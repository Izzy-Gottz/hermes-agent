...
def _model_background_value(args: dict, parent_agent=None) -> bool:
    ...
    within its own turn — as is one that arrived over the tool bridge, whose caller is holding an open MCP call and
    will only ever see this function's return value. The live path is ``run_agent._dispatch_delegate_task``; this
    mirrors it for the rare case the intercept is bypassed, and the two must not drift. Direct Python callers keep
