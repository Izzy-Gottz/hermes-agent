...
def _required_commands(frontmatter: Dict[str, Any]) -> List[str]:
    """``prerequisites.commands`` as a clean list (a bare string counts as one)."""
    prereqs = frontmatter.get("prerequisites")
    raw = prereqs.get("commands") if isinstance(prereqs, dict) else None
...
def _skill_readiness(frontmatter: Dict[str, Any], skill_name: str) -> Tuple[dict, dict]:
    ...
    required_commands = _required_commands(frontmatter)
    ...
    # A skill that shells out to a binary this machine does not have cannot
    # work — and until now nothing ever checked. Both ``required_commands``
    # and ``missing_required_commands`` were hard-coded to ``[]`` in the
    # result below, while ``cron/scheduler.py::_preflight_check_skills`` was
    # already written to read them and name the missing command in its
    # refusal. So the consumer existed, the producer always said "nothing
    # missing", and the check could not fail: a job attached to such a skill
    # was dispatched, spent an LLM call, and died at the shell.
    #
    ...
    # credential-file check above. Setting ``setup_needed`` here relabels the
    # skill for every consumer of this payload, and a declared command is not
    # always the blocker its absence looks like: `agentmail` declares one and
    # keeps its API key optional precisely so the CLI self-signup path stays
    # open. The refusal belongs where the cost is real and irreversible —
    # `cron/scheduler.py::_preflight_check_skills`, which blocks a scheduled
    # job on a missing command directly rather than only when some OTHER
    # requirement already said setup_needed.
