...
_BROWSER_APPS = ("google chrome", "chromium", "microsoft edge", "brave browser",
                 "arc", "vivaldi", "opera", "comet")

# How much a page actually costs, measured on this Mac against an isolated
# Chromium on en.wikipedia.org/wiki/Accessibility — because the published
# guidance for this ("a snapshot is 200-400 tokens vs 3,000-5,000 for a
# screenshot", Playwright MCP) is true of a small page and badly wrong of a
# real one:
#
#     semantic_v2, whole page   63,309 B   ~15,800 tokens   1.6 s
#       of which outline        11,785 B
#       of which action refs    28,778 B   (164 refs; 41 of them in-viewport)
#       of which content refs   20,937 B
#     dom_refs_v1, whole page   34,622 B   ~ 8,700 tokens   0.9 s
#     semantic_v2 + query        7,533 B   ~ 1,900 tokens   2.6 s
#
# A SOM screenshot of the same window is ~1,568 image tokens. So an unscoped
# snapshot is TEN TIMES a screenshot and reading a whole page this way would
# be slower than the thing it replaces. The browser path wins on ACTING —
# exact refs, no coordinate guessing, no focus steal — not on bulk reading.
#
# Hence: scoped by default, and `full` has to be asked for by name.
# Budgets, set so the DEFAULT read is cheaper than the screenshot it replaces
# rather than four times it — which is what an untrimmed "just read the page"
# measured at. See the sizes above.
_BROWSER_REF_BUDGET = 25
_BROWSER_OUTLINE_BUDGET = 4000


def _browser_window(backend, args):
    """Resolve (pid, window_id) for the browser window to read.

    Explicit pid/window_id win; then `app`; then the frontmost browser
    window. Returns (pid, window_id, error_json_or_None).
    """
    pid, wid = args.get("pid"), args.get("window_id")
    if pid is not None and wid is not None:
        return int(pid), int(wid), None
    want = (args.get("app") or "").strip().lower()
    try:
        windows = backend.list_windows()
    except Exception as e:
        return None, None, json.dumps({"error": f"could not list windows: {e}"})
    def _is_browser(w):
        name = (w.get("app_name") or "").strip().lower()
        if want:
    ...
    if not cands:
        return None, None, json.dumps({
            "error": ("no browser window matched"
                      + (f" `{args.get('app')}`" if want else "")),
            "code": "no_browser_window",
            "hint": ("`browser_read` drives Chromium through the DevTools "
                     "protocol. If the browser is not running, open it first "
...
def _browser_trim(body, args):
    """Shrink a semantic_v2 snapshot to what acting needs.

    `outline` is the readable tree and stays. Action refs are kept
    viewport-first up to a budget. `content_refs` — the page prose, a third
    of the payload — is dropped unless the caller asked to read text.
    """
    want_text = bool(args.get("include_text"))
    full = bool(args.get("full"))
    out = {k: v for k, v in body.items()
           if k in ("status", "target_id", "tab_id", "page")}
    outline = body.get("outline") or ""
    if not full and len(outline) > _BROWSER_OUTLINE_BUDGET:
        cut = outline.rfind("\n", 0, _BROWSER_OUTLINE_BUDGET)
        out["outline"] = outline[:cut if cut > 0 else _BROWSER_OUTLINE_BUDGET]
        out["outline_truncated"] = True
        out["hint"] = ("This page is larger than one read. Ask for the part "
                       "you want — query='Send', or scope_ref on a ref above "
                       "— rather than full=true, which is ten times the "
                       "tokens of a screenshot.")
    else:
        out["outline"] = outline
    refs = body.get("refs") or []
    if not full:
        ordered = sorted(refs, key=lambda r: {
            "in_viewport": 0, "near_viewport": 1}.get(r.get("visibility"), 2))
        kept = ordered[:_BROWSER_REF_BUDGET]
        if len(kept) < len(refs):
            out["refs_omitted"] = len(refs) - len(kept)
        refs = kept
    out["refs"] = refs
    if want_text or full:
        out["content_refs"] = body.get("content_refs") or []
    elif body.get("content_refs"):
        out["text_available"] = len(body["content_refs"])
    snap = body.get("snapshot") or {}
    if snap.get("continuation"):
        out["continuation"] = snap["continuation"]
    if snap.get("complete") is False:
        out["complete"] = False
    return out


def _browser_read(backend, args):
    """Read a browser tab over CDP: outline + typed action refs, no pixels."""
    bind_fn = getattr(backend, "browser_bind", None)
    if bind_fn is None:
        return json.dumps({
            "error": "this backend cannot drive a browser over CDP",
            "hint": "capture(mode='som') then click still works.",
        })
    target_id, tab_id = backend.browser_targets()
    rebind = bool(args.get("rebind")) or not (target_id and tab_id)
    if rebind:
        pid, wid, err = _browser_window(backend, args)
        if err is not None:
            return err
        bound = bind_fn(pid=pid, window_id=wid)
        if bound.get("status") != "ok":
            refusal = bound.get("refusal") or {}
            code = refusal.get("code") or "browser_bind_refused"
            out = {"error": refusal.get("message") or "the browser refused binding",
                   "code": code}
            if code == "browser_consent_required":
                out["hint"] = (
                    "Reading this browser over DevTools exposes its live pages, "
                    "cookies and storage, so cua-driver requires the owner to "
                    "turn it on once: set `computer_use.grant_existing_profile: "
                    "true` in ~/.hermes/config.yaml. Until then, use "
                    "capture(mode='som') and click as before.")
            else:
                out["hint"] = ("Fall back to capture(mode='som') and click — "
                               "the screen route always works.")
            return json.dumps(out)
        target_id, tab_id = backend.browser_targets()
    body = backend.browser_snapshot(
        target_id=target_id, tab_id=tab_id,
        query=args.get("query"), scope_ref=args.get("scope_ref"),
        continuation=args.get("continuation"))
    if body.get("status") != "ok":
        refusal = body.get("refusal") or {}
        return json.dumps({
            "error": refusal.get("message") or "the browser refused the read",
            "code": refusal.get("code") or "browser_read_refused",
            "hint": ("The tab may have navigated away. Retry with rebind=true, "
                     "or fall back to capture(mode='som')."),
        })
    return json.dumps(_browser_trim(body, args))
...
def _dispatch(backend: ComputerUseBackend, action: str, args: Dict[str, Any]) -> Any:
    ...
    if action == "browser_read":
        return _browser_read(backend, args)
