...
from tools.computer_use.cua_backend_parse import (
    _apps_from_windows, _degraded_reason_from, _image_dimensions_from_bytes, _image_from_tool_result, _ingest_windows,
    _is_placeholder_id,
...
class _CaptureMixin:
    ...
    def _load_windows(self) -> List[Dict[str, Any]]:
        ...
        This is the *picker*: ``on_screen_only: True`` narrows to the active Space because you cannot
        click across a Space. ``list_windows`` (the model's discovery tool) deliberately does not use it."""
    ...
    def _load_windows_all_spaces_checked(self) -> Tuple[List[Dict[str, Any]], bool]:
        ...
        """
        try:
            out = self._call_capture_tool("list_windows", {"on_screen_only": False, "session": self._session_id})
    ...
    def _no_match_reason(self, app: str) -> str:
        ...
        """
        try:
            elsewhere = self._match_windows_for_app(self._load_windows_all_spaces(), app)
    ...
    def _capture_vision(self) -> Tuple[Optional[str], Optional[str], List[UIElement], str, Optional[str]]:
        ...
        png_b64, image_mime_type, window_title, degraded_reason = None, None, "", None
        ...
        return png_b64, image_mime_type, [], window_title, degraded_reason

    def _capture_window_state(self) -> Tuple[Optional[str], Optional[str], List[UIElement], str, Optional[str]]:
        """AX tree + screenshot. Returns ``(png_b64, mime, elements, window_title, degraded_reason)``."""
        ...
        snap = (gws_out.get("structuredContent") or {}).get("snapshot_id")
        self._snapshot_id = snap if isinstance(snap, str) and snap else None
        return *_image_from_tool_result(gws_out), elements, window_title, _degraded_reason_from(gws_out)
