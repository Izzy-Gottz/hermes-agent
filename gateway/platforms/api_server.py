...
async def _abandon_agent_turn(
    ...
    *,
    ...
    The one sequence every abandonment path in the SSE writers shares — now
    owned by upstream's ``_abandon_agent_task`` (same steps: hard interrupt,
    reap the turn's background processes (#76115), cancel the wrapper).
    Kept under this name so the disconnect watcher and its tests have one
    seam. ``wait`` awaits the cancelled wrapper (client-disconnect paths) or
    leaves it (the server-side ``CancelledError`` path, which must re-raise
    promptly).
    """
    await _abandon_agent_task(
        agent_ref, agent_task, reason, reap_source=source, await_cancel=wait)
...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    ...
    def _create_agent(
        ...
        restrict_toolsets: Optional[List[str]] = None) -> Any:
        """Create an AIAgent from the gateway runtime config + platform toolsets.
        ``restrict_toolsets`` (X-Hermes-Enabled-Toolsets) narrows the RESOLVED toolset for this
        agent only — never widens; see the filter below.
    ...
    async def _run_agent(
        ...
        restrict_toolsets: Optional[List[str]] = None, status_callback=None) -> tuple:
        """Create an agent and run one turn in a thread executor -> ``(result, usage)``.
        ``restrict_toolsets`` narrows this turn's toolset (narrow-only, fail-closed);
        ``status_callback`` receives run_agent's lifecycle/warn notices, which have no other
        home on this platform (the status adapter's send() is a stub here).
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    # Lifecycle status (rate-limit warnings, compaction notices) has no home on
                    # this platform otherwise: the status adapter's send() is a stub here, so
                    # run_agent._emit_status fires into nothing. Set before agent_ref so a status
                    # emitted during setup is not lost.
