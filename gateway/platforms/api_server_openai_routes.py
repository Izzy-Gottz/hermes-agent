...
class OpenAICompatRoutesMixin:
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if _restrict:
            # Opt-in, narrow-only: a client (the learn seam's memory-only save turn, or a
            # read-only gather turn) shrinks THIS turn's toolset to a subset of what api_server
            # exposes. Compute the set that will ACTUALLY be applied — the requested names
            # intersected with what api_server exposes — pass it to the agent (_create_agent
            # re-applies the same narrow-only filter), and echo it on the response as
            # X-Hermes-Applied-Toolsets so a client can FAIL CLOSED when a gateway that predates
            # this feature silently ignores the header. Breaks the "lethal trifecta" for
            # connector-derived data.
        ...
        _applied_headers = (
            {"X-Hermes-Applied-Toolsets": ",".join(_applied_toolsets)}
            if _applied_toolsets is not None else {})
        run_kwargs = dict(
        ...
        if stream:
            ...
            def _on_status(event_type, message):
                ...
                ``event_type`` is run_agent's own vocabulary — ``lifecycle`` for _emit_status,
                ``warn`` for _emit_warning — and is passed through rather than collapsed, so a
                client can treat a warning differently from a notice without parsing the text.
                """
                text = str(message or "").strip()
                if not text:
                    return
                _stream_q.put_threadsafe(("__status__", {
                    "kind": str(event_type or "lifecycle"), "text": text}))
            ...
            agent_task, agent_ref = self._spawn_stream_agent(
                ...
                tool_complete_callback=_on_tool_complete, status_callback=_on_status,
                **run_kwargs)
            ...
            return await self._write_sse_chat_completion(
                ...
                gateway_session_key=gateway_session_key, extra_headers=_applied_headers)
        ...
        # Echo the actually-applied toolset so a restriction-aware client can confirm the turn
        # was narrowed (and refuse to proceed otherwise).
        response_headers.update(_applied_headers)
    ...
    async def _write_sse_chat_completion(
        ...
        gateway_session_key: str = None, extra_headers: Optional[Dict[str, str]] = None,
    ) -> "web.StreamResponse":
        """Stream ``chat.completion.chunk`` frames from the agent's delta queue. On client
        disconnect — noticed by a failed write OR by the disconnect watcher while the agent is
        inside a tool call and nothing is written — the agent is interrupted (stops LLM calls),
        then its task wrapper cancelled; once only."""
        from gateway.platforms.api_server import (
            _SSE_CLIENT_GONE, _SSEClientGone, _SSEDisconnectWatcher, _chat_usage_payload, _sse_frame)
        response = await self._prepare_sse_response(
            request, session_id, gateway_session_key, extra_headers)
        # Notices the client leaving while the agent is inside a tool call and nothing is
        # being written (see _SSEDisconnectWatcher).
        ...
        try:
            ...
            async for delta in _iter_stream_items(stream_q, agent_task, response):
                ...
                if delta is _SSE_CLIENT_GONE:
                    # The watcher saw the transport close; take the same path a failed write would.
                ...
                if isinstance(delta, tuple) and len(delta) == 2 and delta[0] == "__tool_progress__":
                    ...
                    # Additive: never a delta, so a client that does not know this event sees
            ...
            try:
                ...
                if watcher.fired:
                    # The watcher cancelled the agent task under us: this is the client
                    # leaving, not the server cancelling.
                    raise _SSEClientGone("SSE client disconnected")
                raise
            ...
            # Noticed either by a failed write or by the watcher; interrupt, reap and cancel
            # exactly once (the watcher logs the disconnect).
            await watcher.fire()
        except Exception:
    ...
    async def _write_sse_responses(
        ...
        """
        from gateway.platforms.api_server import (
            _SSE_CLIENT_GONE, _SSEClientGone, _SSEDisconnectWatcher, _abandon_agent_task,
        ...
        try:
            ...
            try:
                ...
                if watcher.fired:
                    # The watcher cancelled the agent task under us: the client left; this is
                    # not a server cancel.
                    raise _SSEClientGone("SSE client disconnected")
                raise
