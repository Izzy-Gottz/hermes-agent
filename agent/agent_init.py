...
def _build_client(agent, api_key, base_url, fallback_model):
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        _init_claude_code_client(agent, api_key, base_url)
    else:
        _init_openai_client(agent, api_key, base_url, fallback_model, _provider_timeout)


def _init_claude_code_client(agent, api_key, base_url):
    """Claude Code subscription runtime — the `claude` CLI owns the network and authenticates with
    its setup-token. No HTTP client, no API key. See agent/claude_code_runtime.py (#25267)."""
    agent.client = None
    agent._client_kwargs = {}
