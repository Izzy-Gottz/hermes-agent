...
def _mark_provider_unhealthy(
    ...
    """Hide one provider endpoint until the TTL expires.

    ``reason`` is not decoration — it decides what the log says happened. This
    function had one message for every caller, *"payment / credit error"*, and
    two of its call sites reach it when a provider has **no credential at
    all**: OpenRouter with no ``OPENROUTER_API_KEY`` and an exhausted key pool,
    and Nous with no ``auth.json``. On a machine where neither was ever
    configured that put "marking nous unhealthy for 60s (payment / credit
    error)" in the gateway log every sixty seconds, three lines above Nous's
    own "no Nous authentication found" — sending anyone who read it to check a
    bill for an account that does not exist. ``reason="unconfigured"`` also
    stops the retry loop being pointless: a missing credential cannot resolve
    itself on a timer, so it is held for :data:`_AUX_UNCONFIGURED_TTL_SECONDS`
    and explained once per process rather than re-announced on every attempt.
    """
    ...
    if ttl is None:
        ttl = _AUX_UNCONFIGURED_TTL_SECONDS if unconfigured else _AUX_UNHEALTHY_TTL_SECONDS
    expires_at = time.time() + ttl
    _aux_unhealthy_until[key] = expires_at
    if unconfigured:
        if label in _aux_unconfigured_reported:
            logger.debug("Auxiliary: %s still unconfigured; skipping it until %s.",
