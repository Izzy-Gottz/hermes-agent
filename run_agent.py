...
class AIAgent(
    ...
    def close(self) -> None:
        ...
        _quietly(self._close_claude_code_session)
    ...
    def _close_claude_code_session(self) -> None:
        """Close the Claude Code subprocess session the same way (#25267)."""
    ...
    def _dispatch_delegate_task(self, function_args: dict) -> str:
        ...
        from tools.delegate_tool import (
            _strip_model_hidden_task_fields, delegate_task as _delegate_task, synchronous_delegation_forced,
        )
        ...
        return _delegate_task(
            ...
            # A call that arrived over the tool bridge from a CLI-owned runtime (claude_code /
            # codex_app_server) is holding an open MCP tool call and will only ever see this
            # return value, so a handle would be a receipt for work it is never shown.
            background=not (getattr(self, "_delegate_depth", 0) > 0) and not synchronous_delegation_forced(),
